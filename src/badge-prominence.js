...
function removeHomeBadge(){document.querySelectorAll('.home-badge-display').forEach(el=>el.remove())}
...
async function run(){if(running)return;running=true;try{style();removeHomeBadge();await loadBadges();await applyChat()}finally{running=false}}
function schedule(){clearTimeout(timer);timer=setTimeout(run,220)}
window.addEventListener('load',()=>setTimeout(run,700));
new MutationObserver(records=>{const external=records.some(r=>[...r.addedNodes,...r.removedNodes].some(n=>!(n.nodeType===1&&n.matches?.('.player-badge-pill'))));if(external)schedule()}).observe(document.documentElement,{childList:true,subtree:true});

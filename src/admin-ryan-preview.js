...
async function ensureRyanTab(){
 ...
 try{
  ...
  btn.onclick=()=>showRyanPreview(profile,btn);
  nav.insertBefore(btn,adminBtn);
 } finally {adding=false}
}
function showRyanPreview(profile,btn){
 ...
 content.querySelector('.send-preview').onclick=async()=>{const textarea=content.querySelector('textarea'),body=textarea.value.trim();if(!body)return;const anonymous=content.querySelector('input[type="checkbox"]').checked;const send=content.querySelector('.send-preview');send.disabled=true;send.textContent='Sending…';const {error}=await supabase.from('private_requests').insert({user_id:sessionStorage.getItem('never')||undefined,sender_name:profile.display_name||'Pastor Ryan',kind,body,anonymous});if(error){alert(error.message);send.disabled=false;send.textContent='Send to Pastor Ryan';return;}content.innerHTML='<section class="private-page"><div class="success-card"><span>✓</span><h2>Sent privately</h2><p>The player-facing Ryan flow worked.</p></div></section>';};
}
